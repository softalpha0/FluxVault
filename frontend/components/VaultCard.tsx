      ...
      <div style={{ padding: 20 }}>
        ...
        {isConnected && (
          <div style={{ background: "#080C14", borderRadius: 10, padding: "12px 14px", marginBottom: 14, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 13, fontWeight: 600, color: "#E2EAF4", fontFamily: "monospace" }}>{balance.toFixed(4)} {asset.symbol}</div>
            </div>
            <div>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          {showHowToGet && (
            <div style={{ marginTop: 6, padding: "12px 14px", borderRadius: 8, background: "#080C14", border: "1px solid rgba(255,255,255,0.04)" }}>
              ...
              {asset.faucetUrl ? (
                <a href={asset.faucetUrl} target="_blank" rel="noopener noreferrer" style={{ display: "inline-block", padding: "7px 16px", borderRadius: 7, background: "rgba(0,212,170,0.1)", border: "1px solid rgba(0,212,170,0.25)", color: "#00D4AA", fontSize: 12, fontWeight: 600, textDecoration: "none" }}>
                  Open Flare Faucet
                </a>
              ) : isConnected ? (
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            ...
            {isConnected && (
              <button onClick={() => setAmount(mode === "deposit" ? balance.toFixed(4) : shares.toFixed(4))} style={{ fontSize: 11, color: asset.color, background: "none", border: "none", cursor: "pointer" }}>MAX</button>
            )}
          </div>
          <input
            type="number"
